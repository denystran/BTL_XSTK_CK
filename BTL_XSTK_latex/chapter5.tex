...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-10-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-15-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-17-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-24-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-25-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-26-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-27-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-28-1.pdf}}
...
tuân theo phân phối chuẩn do có p-value bé hơn mức ý nghĩa 5\%, còn các
nhóm còn lại tuân theo phân phối chuẩn do có giá trị p-value lớn hơn mức
ý nghĩa 5\%. Vì điều kiện phân phối chuẩn bị vi phạm, mô hình ANOVA và
phép hậu kiểm Tukey HSD sẽ không còn độ tin cậy và có thể dẫn đến sai
lệch. Do đó, ta quyết định hủy bỏ ANOVA và chuyển sang sử dụng kiểm định
phi tham số. 

\subsubsection{3.3. Kiểm định phi tham số
Kruskal-Wallis}
Kiểm định Kruskal-Wallis dùng để so sánh trung vị của
các nhóm thay vì trung bình, giúp mô hình không bị ảnh hưởng bởi việc dữ liệu không tuân theo phân phối chuẩn.\\ 
\(H_0\): Trung vị độ nhám của 5 mức layer\_height là hoàn toàn giống nhau.\\
\(H_1\): Tồn tại ít nhất một mức tạo ra trung vị độ nhám khác biệt.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{kruskal\_model }\OtherTok{\textless{}{-}} \FunctionTok{kruskal.test}\NormalTok{(roughness }\SpecialCharTok{\textasciitilde{}}\NormalTok{ layer\_height, }\AttributeTok{data =}\NormalTok{ data)}
\FunctionTok{print}\NormalTok{(kruskal\_model)}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  roughness by layer_height
## Kruskal-Wallis chi-squared = 35.282, df = 4, p-value = 4.066e-07
\end{verbatim}

Ta thấy giá trị p-value = 9.944e-07 nhỏ hơn mức ý nghĩa 5\%.
\(\Rightarrow\) Bác bỏ \(H_0\), chấp nhận \(H_1\). 
\(\Rightarrow\) Có sự khác biệt về trung vị độ nhám giữa 5 nhóm chiều cao lớp in.
\subsubsection{3.4. Phân tích sâu sau kiểm định (So sánh bội Post-hoc)}

Để tìm ra chính xác cặp thông số nào có sự khác biệt, ta sử
dụng kiểm định Wilcoxon với hiệu chỉnh Bonferroni

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{posthoc\_result }\OtherTok{\textless{}{-}} \FunctionTok{pairwise.wilcox.test}\NormalTok{(data}\SpecialCharTok{$$}\NormalTok{roughness, data}\SpecialCharTok{$}\NormalTok{layer\_height, }
                                       \AttributeTok{p.adjust.method =} \StringTok{"bonferroni"}\NormalTok{)}
\FunctionTok{print}\NormalTok{(posthoc\_result)}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  data$roughness and data$layer_height 
## 
##      0.02    0.06    0.1     0.15   
## 0.06 0.00130 -       -       -      
## 0.1  0.00011 0.27571 -       -      
## 0.15 0.00011 0.03605 1.00000 -      
## 0.2  0.00011 0.00032 0.24584 0.09537
## 
## P value adjustment method: bonferroni
\end{verbatim}

Kết luận: Dựa vào ma trận p-value sau khi hiệu chỉnh Bonferroni, ta thấy mức layer\_height = 0.02 có sự khác biệt rõ rệt (p-value \textless{} 0.05) so với các mức 0.15 và 0.2. Chiều cao lớp in càng thấp thì độ nhám bề mặt càng nhỏ. Muốn độ nhám thấp nhất thì layer\_height cũng phải thấp nhất là 0.02, muốn độ nhám giảm đi thì chiều cao lớp in layer\_height cũng phải giảm đi.
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-32-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-36-1.pdf}}
...
\begin{figure}[htbp]
    \centering
    \pandocbounded{\includegraphics[keepaspectratio]{unnamed-chunk-37-1.pdf}}
